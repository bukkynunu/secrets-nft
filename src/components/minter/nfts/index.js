...
const NftList = ({ minterContract, name }) => {
  ...
  const getAssets = useCallback(async () => {
    try {
      ...
      const allNfts = await getSecrets(minterContract);
      if (!allNfts) return;
      setNfts(allNfts);
    } catch (error) {
      console.log({ error });
  ...
  const addNft = async (data) => {
    try {
      ...
      await addSecret(minterContract, performActions, data);
      toast(<NotificationSuccess text="Updating NFT list...." />);
      getAssets();
    } catch (error) {
      console.log({ error });
      toast(<NotificationError text="Failed to create your secret" />);
    } finally {
      setLoading(false);
    }
  };

  const ownSecrets = async (index, tokenId) => {
    try {
      setLoading(true);
      await ownSecret(minterContract, index, tokenId, performActions);
      getAssets();
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const disownSecrets = async (index) => {
    try {
      setLoading(true);
      await disownSecret(minterContract, index, performActions);
      getAssets();
    } catch (error) {
      console.log(error);
    } finally {
            ...
            <Row xs={1} sm={2} lg={3} className="g-3  mb-5 g-xl-4 g-xxl-5">
              ...
              {nfts.map((_nft) => (
                <Nft
                  ...
                  ownSecret={() => ownSecrets(_nft.index, _nft.tokenId)}
                  disownSecret={() => disownSecrets(_nft.tokenId)}
                  send={sendNft}
                  nft={{
                    ..._nft,
                  }}
